...
import com.eternallove.demo.zuccfairy.ui.customview.pageradapter.RecycleAdapter;
...
public class DayPager extends BasePager {
    ...
    public void initData() {
        ...
        list = FairyDB.getInstance(mActivity).getDataByDay(calendar);
        ...
        BusProvider.getInstance().toObserverable()
                .subscribe(event -> {
                    if (event instanceof Events.DayClickedEvent) {
                        ...
                        list = FairyDB.getInstance(mActivity).getDataByDay(clickedEvent.getCalendar());
